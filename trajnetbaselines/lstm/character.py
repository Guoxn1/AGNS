...
class Trainer(object):
    ...
    def attack_batch(self, xy, goals, batch_split, thread_index=None, scene_id = None):
        ...
        noise_1 = torch.zeros(12, requires_grad=False).to(self.device)
        noise = torch.zeros(6, requires_grad=True)
        
        noise.data = self.clamp(noise, temp_barrier).to(self.device)
        ...
        while (counter != 0):
            ...
            if self.perturb_all == 'true':
                perturbed_path += torch.cat((noise_1, noise), dim=0) # 这里
            else:
                perturbed_path[2:-2] += torch.cat((noise_1, noise), dim=0) # 这里
        ...
        perc = self.getCharacter(outputs.cpu().detach().numpy(),best_outputs_perturbed.cpu().detach().numpy())
        self.character_number += perc
        if(perc!=0):
            self.char_sum +=1
        print(self.character_number/self.char_sum)
        return 0, 0, 0
        while_end = time.time()
    ...
    def getCharacter(self,ouputs,perbued):
        # 假设 A 和 B 是你的 19*6*2 的轨迹矩阵
        # A = np.random.random((19, 6, 2))
        # B = np.random.random((19, 6, 2))

        # 提取第8到第16帧，第一个物体的坐标 (Python索引从0开始，所以是7到15)
        A_segment = ouputs[7:16, 0, :]  # 9*2矩阵
        B_segment = perbued[7:16, 0, :]  # 9*2矩阵

        # 计算B和A的坐标差，得到一个9*2的矩阵
        difference = B_segment - A_segment
        # 计算每帧坐标的大小（欧几里得距离），得到9个坐标的大小
        sizes = np.linalg.norm(difference, axis=1)
        
        # 提取最后五帧的数据
        last_five_sizes = sizes[-3:]
        # 计算最后一个坐标的大小（最后一帧的大小）
        last_size = sizes[-1]

        # 计算最后五帧的总大小
        total_last_five_size = np.sum(last_five_sizes)

        # 计算最后一个坐标的大小占最后五帧总大小的百分比
        if total_last_five_size==0:
            return 0
        percentage = (last_size / total_last_five_size) * 100
