...
class Trainer(object):
    def __init__(self, model=None, model1 = None, criterion='L2', lr=None, barrier=1, show_limit=30,
        ...
        self.model1 = model1 if model1 is not None else LSTM()
        ...
        self.models1 = [copy.deepcopy(self.model1) for _ in range(self.threads_limit)]
    ...
    def attack_batch(self, xy, goals, batch_split, thread_index=None, scene_id = None):
        ...
        local_model1 = self.models1[thread_index]
        ...
        noise_1 = torch.zeros(8, requires_grad=False).to(self.device)
        noise = torch.zeros(10, requires_grad=True)

        noise.data = self.clamp(noise.data, temp_barrier).to(self.device)
        ...
        while (counter != 0):
            ...
            if self.perturb_all == 'true':
                perturbed_path += torch.cat((noise_1, noise), dim=0)
            else:
                perturbed_path[2:-2] += torch.cat((noise_1, noise), dim=0)
            ...
            rel_outputs_perturbed1, temp_outputs_perturbed,_,_ = local_model1(perturbed_observation, goals.clone(), batch_split,n_predict=12)
            outputs_perturbed1 = temp_outputs_perturbed[0]
            # Clear NaNs
            ...
            outputs_perturbed_nan1 = torch.isnan(outputs_perturbed1)
            for i in range(len(outputs_perturbed1)):
                for j in range(len(outputs_perturbed1[0])):
                    if any(outputs_perturbed_nan1[i, j].tolist()):
                        outputs_perturbed1.data[i, j] = 10000
                        outputs_perturbed1[i, j].detach()

            distances, timeCollo, frame_index, neighbor_index, score = self.getFrame(agents_count, outputs_perturbed)
            distances1, timeCollo1, frame_index1, neighbor_index1, score1 = self.getFrame(agents_count, outputs_perturbed1)
            # Collision Loss
            loss = None
            if self.collision_type == 'soft':
                w_sf = sf(w.view(-1)).view(self.pred_length, agents_count-1).to(self.device)
                A = (w_sf * torch.tanh(timeCollo)).view(-1)
                loss = torch.sum(A[~torch.isnan(A)]) +  self.reg_noise * torch.norm(torch.cat((noise_1, noise), dim=0), 2, dim=0) - self.reg_w * torch.norm(w_sf.view(-1), 2, dim=0)
            ...
            if loss < best_loss_by_now:
                ...
                if loss.cpu() < torch.tanh(torch.Tensor([3])) and self.collision_type == 'soft':
                ...
                best_score_by_now1 = score1.item()
        ...
        did_collide = False
        print(best_score_by_now)
        print(best_score_by_now1)
        if best_score_by_now < collision_done_barrier and best_score_by_now1 < 0.3:
    ...
    def getFrame(self, agents_count, outputs_perturbed):
        # Each Neighbors Distance to The Main Agent

        distances = (torch.sqrt(torch.sum((torch.square(outputs_perturbed[-self.pred_length:]- outputs_perturbed[-self.pred_length:, 0].repeat_interleave(agents_count, 0).reshape(self.pred_length, agents_count, 2))[:, 1:]), dim=2)))

        time_series_data = outputs_perturbed[-self.pred_length:]
        velocities = torch.zeros((self.pred_length, agents_count-1)).to(self.device)
        for i in range(0, self.pred_length-1):
            for j in range(1, agents_count):
                # 计算节点j相对于节点1的距离变化
                dx_curr = time_series_data[i, j, 0] - time_series_data[i, 0, 0]
                dy_curr = time_series_data[i, j, 1] - time_series_data[i, 0, 1]
                dx_prev = time_series_data[i + 1, j, 0] - time_series_data[i + 1, 0, 0]
                dy_prev = time_series_data[i + 1, j, 1] - time_series_data[i + 1, 0, 1]

                # 计算节点j相对于节点1的靠近速度
                dist_curr = torch.sqrt(dx_curr ** 2 + dy_curr ** 2)
                dist_prev = torch.sqrt(dx_prev ** 2 + dy_prev ** 2)
                velocities[i, j - 1] =  dist_curr - dist_prev  # 计算靠近速度

        timeCollo = distances - velocities
        # Score
        ...
        return distances,timeCollo, frame_index, neighbor_index, score
...
def main(epochs=10):
    ...
    parser.add_argument('--barrier', default=0.2, type=float,
    ...
    # d_pool  type=d_pool  state:d_pool
    # satt   type = satt  state: lstm
    # lstm  type = d_pool  state :lstm
    parser.add_argument('--type', default='d_pool',
                        choices=('s_lstm', 'd_pool', 's_att','hiddenstatemlp'),
                        help='type of LSTM to train')
    # 转移的模型
    parser.add_argument('--type1', default='d_pool',
                        choices=('s_lstm', 'd_pool', 's_att', 'hiddenstatemlp'),
                        help='type of LSTM to train')
    ...
    # 转移的path
    parser.add_argument('--models_path1', default='trajnetbaselines/lstm/Target-Model/d_pool.state',
                        help='the directory of the model')
    ...
    parser.add_argument('-o', '--output', default='./out/tmp/',
                        help='output file')
    ...
    parser.add_argument('--reg_w', default=0.7, type=float,
    ...
    parser.add_argument('--speed_up', default='true',
                        choices=('true', 'false'),
                        help='speed up?')
    ...
    hyperparameters.add_argument('--layer_dims', type=int, default=1024, nargs='*',
    ...
    if args.type1 == 'hiddenstatemlp':
        pool1 = HiddenStateMLPPooling(hidden_dim=args.hidden_dim, out_dim=args.pool_dim,
                                     mlp_dim_vel=args.vel_dim)
    elif args.type1 == 'd_pool':
        pool1 = NN_LSTM(n=args.neigh, hidden_dim=args.hidden_dim, out_dim=args.pool_dim)
    elif args.type1 == 's_att':
        pool1 = SAttention(hidden_dim=args.hidden_dim, out_dim=args.pool_dim)
    ...
    lstm_generator = LSTMGenerator(embedding_dim=args.coordinate_embedding_dim, hidden_dim=args.hidden_dim,
                                   pool=pool1, goal_flag=args.goals, goal_dim=args.goal_dim, noise_dim=args.noise_dim,
    ...
    if args.discriminator:
        lstm_discriminator = LSTMDiscriminator(embedding_dim=args.coordinate_embedding_dim,
                                               hidden_dim=args.hidden_dim, pool=pool1,
                                               goal_flag=args.goals, goal_dim=args.goal_dim)

    # GAN model
    model1 = SGAN(generator=lstm_generator, discriminator=lstm_discriminator,
                 add_noise=args.add_noise, k=args.k)
    
    
    # Load model
    load_address = args.models_path
    load_address1 = args.models_path1
    print("Loading Model Dict from ", load_address)
    print("Loading Model1 Dict from ", load_address1)
    ...
    with open(load_address1, 'rb') as f:
        checkpoint1 = torch.load(f)
    pretrained_state_dict1 = checkpoint1['state_dict']
    model1.load_state_dict(pretrained_state_dict1, strict=False)
    ...
    trainer = Trainer(model,model1, lr=args.lr, device=args.device, barrier=args.barrier, show_limit=args.show_limit,
