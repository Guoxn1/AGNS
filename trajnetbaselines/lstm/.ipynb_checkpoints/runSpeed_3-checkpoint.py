...
class Trainer(object):
    ...
    def attack_batch(self, xy, goals, batch_split, thread_index=None, scene_id = None):
        ...
        noise_1 = torch.zeros(6, requires_grad=False).to(self.device)
        noise = torch.zeros(8, requires_grad=True)
        noise_2 = torch.zeros(4, requires_grad=False).to(self.device)
        ...
        while (counter != 0):
            ...
            if self.perturb_all == 'true':
                perturbed_path += torch.cat((noise_1, noise, noise_2), dim=0) # 这里
            else:
                perturbed_path[2:-2] += torch.cat((noise_1, noise, noise_2), dim=0) # 这里
            ...
            if self.collision_type == 'soft':
                ...
                loss = torch.sum(A[~torch.isnan(A)]) +  self.reg_noise * torch.norm(torch.cat((noise_1, noise, noise_2), dim=0), 2, dim=0) - self.reg_w * torch.norm(w_sf.view(-1), 2, dim=0)
